...
export class AuthService {
    ...
    async validarLogin(dto: LoginDto) {
        ...
        try {
            ...
            // 3. CONSULTA DE MENÚS (puro, solo navegación).
            const sqlMenu = `
                SELECT
                    m.menu_id,
                    m.menu_padre_id,
            ...
            // 4. CONSULTA DE PERMISOS GRANULARES (por tabla, top-level).
            ...
            if (isDomainException(error) || error instanceof UnauthorizedException) {
                throw error;
            }

            const errorMessage = getErrorMessage(error);
            this.logger.error(`Error inesperado en autenticación para usuario '${username}': ${errorMessage}`, getErrorStack(error));

            throw new UnauthorizedException(
                'Error al procesar la autenticación. Intente nuevamente.'
            );
        }
    }
}
