...
export class CuisService extends BaseService {
    ...
    async update(id: number, dto: UpdateCuiDto, usuarioId: number): Promise<CuiResponseDto> {
        return runInTransaction(this.dataSource, async (manager) => {
            ...
            if (dto.codigo_cuis) {
                dto.codigo_cuis = dto.codigo_cuis.trim().toUpperCase();
            }
            ...
            const nuevoSucursalId = dtoProcesado.sucursal_id ?? cuisActual.sucursal_id;
            ...
            if (dtoProcesado.sucursal_id !== undefined &&
                dtoProcesado.sucursal_id !== cuisActual.sucursal_id) {
                validaciones.push(
            ...
            if (dtoProcesado.punto_venta_id !== undefined &&
                dtoProcesado.punto_venta_id !== cuisActual.punto_venta_id &&
            ...
            if (dtoProcesado.codigo_cuis !== undefined &&
                dtoProcesado.codigo_cuis !== cuisActual.codigo_cuis) {
                validaciones.push(
                    this.unicidadValidador.validarUnicidad({
                        tabla: this.nombreTabla,
                        campos: [
                            { nombre: 'codigo_cuis', valor: dtoProcesado.codigo_cuis }
                        ],
                        idExcluir: id,
                        estadosValidos: [...ESTADOS_VIVOS],
                        campoPk: this.campoPK,
            ...
            Object.assign(cuisActual, dtoProcesado);
