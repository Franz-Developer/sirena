...
4. **Parámetros de Paginación y Ordenamiento Estándar (Obligatorios al final):**
   @IsOptional()
    ...
    @IsOptional()
    @IsString({ message: 'El campo sortField debe ser una cadena de texto.' })
    sortField?: string = 'PRIMARY_KEY_id';   <-- siempre ordenado por primary_key 
...
6. **Nomenclatura estricta de la Clase:** El nombre de la clase debe estar en **singular** y seguir el formato `Find[NombreModulo]QueryDto` (ej. `FindDominioQueryDto`, no `FindDominiosQueryDto`).
7. **Checklist final obligatorio:** Antes de generar el código, valida que el `sortField` por defecto utilice estrictamente el nombre de la llave primaria de la tabla analizada (ej. `dominio_id` en lugar de una genérica).
8. **Formato de Código:** Todo el código generado debe ir estrictamente dentro de las comillas invertidas de Markdown o backticks.
...
7. **Nomenclatura estricta de la Clase:** El nombre de la clase debe estar en **singular** (ej. `DominioResponseDto` o `TipoCambioResponseDto`, nunca en plural).
8. **Checklist final obligatorio:** Verifica que todos los campos de auditoría (`fecha_registro`, etc.) y decimales utilicen el operador exacto de nulabilidad que le corresponda según el DDL.
9. **Formato de Código:** Todo el código generado debe ir estrictamente dentro de las comillas invertidas de Markdown o backticks.
