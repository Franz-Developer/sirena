...
Explicación:
En app/pages/index.vue, se usa useHead({ title: ${config.public.appName} | Login }); pero config se define después con const config = useRuntimeConfig();. En <script setup>, el orden de ejecución es secuencial, pero useHead se llama antes de que config esté definida, lo que provocará un error de referencia.
